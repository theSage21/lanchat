...
class Node:
            ...
            while self.alive:
                try:
                    ...
                    with self.__client_list_lock:
                        ...
                        stats('User count: {}'.format(len(self.clients)))
    ...
    def __shutdown(self):
        ...
        if self.mode == 'c':
            notice('Telling server that im leaving')
            utils.quit(self.__s)
        else:  # server
            try:
                with self.__client_list_lock:
                ...
                notice('Assigning new server for network')
                utils.assume_server(new_server)
                # tell others to quit
                notice('Telling everyone Im leaving')
                with self.__client_list_lock:
                    for com in self.clients:
                        utils.quit(com)
        notice('LanChat is closing. Use again')
        self.__s.close()
    ...
    def __make_client(self):
        ...
        notice('Making client, getting server connection')
        self.mode = 'c'
        addr = utils.get_existing_server_addr()
